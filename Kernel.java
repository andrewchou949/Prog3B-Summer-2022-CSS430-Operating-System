...
// Class: CSS430
// ------------------------------------------Kernel.java-------------------------------------------------
// This Kernel.java provides all the numbers coding to determine each condition. It also characterizes
//    the behavior for each of the case determined below.
//    Modifications were made to case "Wait" and "Exit" only. Other than these are of the source code
//	  	which belong to the original java file creator.
//
// -------------------------------------------------------------------------------------------------------
...
public class Kernel
{
	...
	case INTERRUPT_SOFTWARE: // System calls
		...
		int threadID = myTcb.getTid();
		// let the current thread sleep in waitQueue under the 
		waitQueue.enqueueAndSleep(threadID);
		...
		int parentThreadID = myTcb.getPid();
		...
		waitQueue.dequeueAndWakeup(parentThreadID, currTid);
